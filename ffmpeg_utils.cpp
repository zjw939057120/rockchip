...
int ffmpeg_utils::push_h264_to_rtsp(const void *data, size_t len, int fps) {
    // 初始化 AVPacket
    av_init_packet(&pkt);
    ...
    pkt.dts = pts_counter;
    pkt.duration = 1;
    pts_counter += 90000 / fps; // 90000 是 H.264 的时间基
    dts_counter += 90000 / fps;

    // 推送数据包
    ret = av_interleaved_write_frame(fmt_ctx, &pkt);
    if (ret < 0) {
        fprintf(stderr, "Error occurred when writing frame: %d\n", ret);
    }
    return ret;
}
